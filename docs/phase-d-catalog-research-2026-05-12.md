...
> **作成**: 2026-05-12 (火) GA 達成日 (オプション C 採用、5/12 20:00 GA、48 日前倒し)
...
> **方針**: 優先度「高」4 系列を最優先確定、「中」4 系列は 5/13-5/31 で並行調査、「低」2 系列は Phase D 第 2 期 (7 月) へ持ち越し可
...
## 3. Phase D 着手タイムライン (GA 達成日 5/12 反映、48 日前倒しでバッファ最大化)

| 期間 | アクション |
|---|---|
| **5/12 (火) 20:00** | 🎊 GA 達成 (EIC 本体サイト記事公開、当初 6/29 → 48 日前倒し) |
| **5/13-5/19** | 優先度「高」4 系列 (US Nonfarm / 米国失業率 / ECB 政策金利 / BoJ 短観) の fetch スクリプト改修、データ取得テスト |
| **5/20-5/26** | 優先度「中」4 系列 (USD/EUR / CPI Food / CPI Energy / 鉄鋼石) の fetch スクリプト改修 + ライセンスリク監修 |
| **5/27-5/31** | catalog.json への 8-10 系列追加 PR + テスト + main 反映 |
...
| # | 系列 | 確認内容 | 期限 |
|---|---|---|---|
| 1 | ECB 政策金利 / USD/EUR | ECB free use と CC BY 4.0 の互換性、出典表記の正確性 | 5/18 (月) (Phase D 着手前) |
| 2 | 中国 PMI | 国家統計局 (NBS) 利用規約、商用利用 + 再配布可否 | 5/22 (金) |
| 3 | 木質燃料指数 | FAO / USDA / 林野庁 の 3 ソース比較、ライセンス互換性 | Phase D 第 2 期着手前 (6/30) |

---

## 6. Phase D 第 1 期 Insight #43-#64 候補リストとの整合性確認

`docs/phase-d-period-1-insight-candidates-2026-06.md` (energy-data-platform repo) の 22 本候補と、本調査の 10 系列の対応関係:

| 新規 catalog 系列 | 使用 Insight | 優先度 |
|---|---|---|
| US Nonfarm Payrolls | #43 米雇用統計 × USD/JPY × JGB 10y | 高 |
| ECB 政策金利 | #48 ECB 政策金利 × USD/EUR × Brent | 高 |
| USD/EUR | #48 (同上) | 中 |
| US CPI Food | #51 米 CPI 食料 vs エネルギー寄与度 | 中 |
| US CPI Energy | #51 (同上) | 中 |
| BoJ 短観 | #53 日銀短観 × JGB 10y × 円安連鎖 | 高 |
| 中国 PMI | #55 中国経済鈍化 × 鉄鋼石 × 石炭 | 中 |
| 鉄鋼石 | #55 (同上) | 中 |
| 米国失業率 | #58 米国失業率 × Fed FF rate × USD/JPY | 高 |
| 木質燃料指数 | #57 バイオマス発電量 × 木質燃料 × CO2 排出 | 低 |

**結論**: 22 本の Insight 候補すべてが新規 catalog 10 系列 (+ 既存 105 系列) でカバー可能。優先度高 4 系列が確定したため、Insight #43 / #48 / #53 / #58 の 4 本は 6/2 着手日から起草可能。優先度低の #57 (木質燃料) は Phase D 第 2 期 (7 月) 持ち越し時に CO2 排出原単位の単軸で再構成も可。

## 7. 自発訂正ログ (L-013)

- **ファイル名の日付不整合**: 大規模告知準備 Day 1 時に `phase-d-catalog-research-2026-05-13.md` で作成したが、本来 GA 達成日 5/12 の成果物として `phase-d-catalog-research-2026-05-12.md` への命名が正しい (Phase D 着手前の調査として 5/12 中に完成させたため)。GA 達成日タスクで `git mv` でリネーム + タイムライン記述を 5/12 GA 反映に更新。

---

*アオ (起草、Phase D 候補リスト連動) + リン (編集、2026-05-12 火曜、🎊 GA 達成日、48 日前倒し版)*

🎯 **Phase D 第 1 期 22 本の Insight 候補のうち、優先度高 4 系列のソース・ライセンスを確定。5/27-5/31 で catalog 追加 PR を作成し、6/2 着手準備を完了させる。** 5/12 GA 達成により 13 日のバッファが確保されているため、優先度中 4 系列のリク監修も余裕を持って完了可能。
