...
export const GLOSSARY_TERMS: GlossaryTerm[] = [
  ...
  {
    slug: "market-splitting",
    name: "市場分断",
    description:
      "JEPXスポットは全国単一の前日オークションだが、約定の結果エリア間の送電量が連系線容量の上限（[[transmission-line-constraint]]）に達すると、その境界で市場が分割され、両側で別々の約定価格＝エリアプライスが付く仕組み。分断がない時間帯は全国同一のシステムプライスで約定する。冬の北海道（上方向・暖房需要と北本連系線）や太陽光余剰の九州（下方向、[[solar-surplus]]）が代表例。エリア間の価格差は送電容量の希少性を映す価格シグナルで、連系線増強の便益評価の根拠にもなる。詳細は Insight「エリアプライスと市場分断の読み方」参照。",
    category: "power",
  },
  {
    slug: "reserve-margin",
    name: "予備率",
    description:
      "（供給力−需要）÷需要で表す、瞬間の供給余裕（kW）の指標。周波数維持と突発トラブル対応のため最低3%が必要とされ、安定的には8%程度が望ましいとされる。予備率5%未満見込みで需給ひっ迫注意報、3%未満見込みで警報（2022年度に段階構造化）。kWh（電力量）の余裕ではなく [[kw-value]] の現在値。ひっ迫は [[jepx-spot]] の高騰や [[imbalance]] の補正として市場にも現れる。実況・見通しは各エリアの「でんき予報」（一般送配電事業者・OCCTO）が一次資料。詳細は Insight「予備率と需給ひっ迫の読み方」参照。",
    category: "power",
  },
  ...
  // ===== 2026-08-11 Insight #107 how-to-read-grid-capacity 連動 +2 用語 =====
  {
    slug: "available-capacity",
    name: "空き容量",
    description:
      "変電所・送電線に追加で接続できる発電設備の容量（MW）。単純な物理的余りではなく、送電設備が1回線故障しても停電させないN-1基準を前提に算定した余力で、かつ既に接続契約を結んだ設備の分を差し引く契約ベース。このため実潮流が空いていても空き容量0となる事象が全国で起き、日本版コネクト＆マネージ（想定潮流の合理化・[[n1-denryu]]・ノンファーム型接続）による解消が図られた。各一般送配電事業者が変電所単位で個別に公表（形式・更新時期は事業者ごとに異なる）。詳細は Insight「空き容量とN-1電制の読み方」参照。",
    category: "power",
  },
  {
    slug: "n1-denryu",
    name: "N-1電制",
    description:
      "N-1電源制限。送電線の1回線故障時に特定電源を自動遮断（電制）する装置の設置を条件に、平常時はN-1基準を超えて送電容量を活用してよいとする仕組み。事故時に自設備が遮断されるリスクを受け入れる代わりに、[[available-capacity]]が0でも系統接続が可能になる。日本版コネクト＆マネージ3手法の中核で、系統情報の公表では変電所ごとに「N-1可否」が示される。[[grid-constraint]] の緩和策。詳細は Insight「空き容量とN-1電制の読み方」参照。",
    category: "power",
  },
];
