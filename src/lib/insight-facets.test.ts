...
describe("Phase C Day 1: insight-facets", () => {
  ...
  test("INSIGHT_RENDERER_MAP: 42 本全 slug がマップに含まれる (Week 1 Day 3 で #42 fuel-cost-decomp 追加)", () => {
    ...
    expect(INSIGHTS.length).toBe(42);
    expect(Object.keys(INSIGHT_RENDERER_MAP)).toHaveLength(42);
  });
  ...
  test("filterInsights: tag=「金融」で 8 件 (Phase B-A Day 13 で確立した macro group のうち macro タグ非依存のもの含め 8 件)", () => {
  ...
  test("filterInsights: domain=finance で 8 件 (金融タグ含む = finance domain 導出)", () => {
