...
describe("Phase C Day 1: insight-facets", () => {
  ...
  test("INSIGHT_RENDERER_MAP: 72 本全 slug がマップに含まれる (5/31 方法論ライブラリ Day 3 #72 forecast-error-baseline-jepx 追加で 71、6/7 #73 population-decline-vs-power-demand 追加で 72)", () => {
    ...
    expect(INSIGHTS.length).toBe(72);
    expect(Object.keys(INSIGHT_RENDERER_MAP)).toHaveLength(72);
  });
  ...
  test("summarizeInsightFacets: domains には all + 出現 domain のみ、count 整合 (5/31 方法論ライブラリ Day 3 #72 forecast-error-baseline-jepx 追加で all 71 / power は「電力」タグで +1、6/7 #73 population-decline-vs-power-demand 追加で all 71 → 72 / power は「電力」タグでさらに +1、finance は不変)", () => {
    const facets = summarizeInsightFacets(INSIGHTS);
    const all = facets.domains.find((d) => d.value === "all");
    expect(all?.count).toBe(72);
