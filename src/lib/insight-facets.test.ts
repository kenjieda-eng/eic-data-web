...
describe("Phase C Day 1: insight-facets", () => {
  ...
  test("INSIGHT_RENDERER_MAP: 75 本全 slug がマップに含まれる (5/31 方法論ライブラリ Day 3 #72 forecast-error-baseline-jepx 追加で 71、6/7 #73 population-decline-vs-power-demand 追加で 72、6/8 #74 lcoe-vs-power-mix 追加で 73、6/9 #75 eu-ets-vs-jp-gx 追加で 74、6/10 #76 eu-ets-allocation-gap-by-country 追加で 75)", () => {
    ...
    expect(INSIGHTS.length).toBe(75);
    expect(Object.keys(INSIGHT_RENDERER_MAP)).toHaveLength(75);
  });
  ...
  test("summarizeInsightFacets: domains には all + 出現 domain のみ、count 整合 (5/31 方法論ライブラリ Day 3 #72 forecast-error-baseline-jepx 追加で all 71 / power は「電力」タグで +1、6/7 #73 population-decline-vs-power-demand 追加で all 71 → 72 / power は「電力」タグでさらに +1、6/8 #74 lcoe-vs-power-mix 追加で all 72 → 73 / power は「電力」タグでさらに +1、6/9 #75 eu-ets-vs-jp-gx 追加で all 73 → 74 / power は「電力」タグでさらに +1、6/10 #76 eu-ets-allocation-gap-by-country 追加で all 74 → 75 / power は「電力」タグでさらに +1、finance は不変)", () => {
    const facets = summarizeInsightFacets(INSIGHTS);
    const all = facets.domains.find((d) => d.value === "all");
    expect(all?.count).toBe(75);
