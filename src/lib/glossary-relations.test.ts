...
describe("/glossary/graph: glossary-relations", () => {
  ...
  test("GLOSSARY_RELATIONS は 70-200 件 (Day 4 拡張 70-100 + Day 6 PM +10 で 109 + Phase D #61 +10 で 119 + Phase D #61 連動 容量市場 +10 で 129 + D-018 需給調整市場 +9 で 138 + Phase 2 Ember 3部作 +20 で 158 + EU ETS 方法論 +12 で 170 + #74 lcoe/lcos +5 で 175 + #100 託送料金 +4 で 179 + #102 市場分断 +3 で 182 + #103 予備率 +3 で 185 + #104 非化石証書 +2 で 187 + #107 空き容量/N-1電制 +4 で 191)", () => {
    expect(GLOSSARY_RELATIONS.length).toBeGreaterThanOrEqual(70);
    expect(GLOSSARY_RELATIONS.length).toBeLessThanOrEqual(200);
    expect(GLOSSARY_RELATIONS.length).toBe(191);
  });
  ...
  test("buildGlossaryGraph: nodes は 80 件、各 degree が正しく集計", () => {
  ...
  test("全 80 用語が少なくとも 1 つの関連 (孤立ノードなし)", () => {
    const g = buildGlossaryGraph();
