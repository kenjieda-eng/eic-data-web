...
describe("/glossary/graph: glossary-relations", () => {
  ...
  test("GLOSSARY_RELATIONS は 70-190 件 (Day 4 拡張 70-100 + Day 6 PM +10 で 109 + Phase D #61 +10 で 119 + Phase D #61 連動 容量市場 +10 で 129 + D-018 需給調整市場 +9 で 138 + Phase 2 Ember 3部作 +20 で 158 + EU ETS 方法論 +12 で 170 + #74 lcoe/lcos +5 で 175 + #100 託送料金 +4 で 179 + #102 市場分断 +3 で 182 + #103 予備率 +3 で 185 + #104 非化石証書 +2 で 187)", () => {
    expect(GLOSSARY_RELATIONS.length).toBeGreaterThanOrEqual(70);
    expect(GLOSSARY_RELATIONS.length).toBeLessThanOrEqual(190);
    expect(GLOSSARY_RELATIONS.length).toBe(187);
  });
  ...
  test("buildGlossaryGraph: nodes は 78 件、各 degree が正しく集計", () => {
    const g = buildGlossaryGraph();
    expect(g.nodes).toHaveLength(78);
    // 手動チェック: jepx-spot は 6 + Day4 拡張 2 (fuel-pass-through / industrial-production) + Day5 午後 2 (yen-denominated-cost / transmission-line-constraint) + Phase D 1 (occto) + Phase D #61 連動 1 (kw-value) + #102 市場分断 1 (market-splitting) + #103 予備率 1 (reserve-margin) = 14 本
    const jepx = g.nodes.find((n) => n.slug === "jepx-spot");
    expect(jepx?.degree).toBe(14);
    // 全 degree の合計 = エッジ数 × 2
  ...
  test("全 78 用語が少なくとも 1 つの関連 (孤立ノードなし)", () => {
    const g = buildGlossaryGraph();
