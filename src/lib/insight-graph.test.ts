...
describe("/insight/network: insight-graph", () => {
  test("buildInsightGraph: nodes は 42 件 (INSIGHTS と同数)、孤立ノードなし", () => {
    const g = buildInsightGraph();
    expect(g.nodes).toHaveLength(INSIGHTS.length);
    expect(g.nodes.length).toBeGreaterThanOrEqual(42);
    // 全 42 本が少なくとも 1 エッジを持つ (孤立ゼロ)
